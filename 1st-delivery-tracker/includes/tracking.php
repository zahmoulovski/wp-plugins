...
function fdt_get_barcode_from_order($order_id) {
    ...
    error_log("Barcode found for order {$order_id}: " . ($result ? $result : 'none'));
    return $result;
}
...
function fdt_get_order_from_barcode($barcode) {
    ...
    $query = $wpdb->prepare(
        ...
        FROM {$wpdb->prefix}postmeta 
        WHERE LOWER(meta_value) = LOWER(%s)
        AND meta_key = '_first_delivery_barcode'
        UNION
        SELECT order_id 
        FROM {$wpdb->prefix}wc_orders_meta 
    ...
    $result = $wpdb->get_var($query);
    error_log("Order found for barcode {$barcode}: " . ($result ? $result : 'none'));
    return $result;
}
    ...
    if (!empty($search_term)) {
        ...
        if ($response && isset($response['result']['Items']) && !empty($response['result']['Items'])) {
            ...
            $state_map = [
                ...
                'Demande d\'enlèvement' => ['step' => 2, 'color' => '#fbbc34'],
                'Demande d\'enlèvement assignée' => ['step' => 2, 'color' => '#fbbc34'],
                'En cours d\'enlèvement' => ['step' => 2, 'color' => '#fbbc34'],
                'Enlevé' => ['step' => 2, 'color' => '#fbbc34'],
